		...
		var url = "http://140.121.196.23:3390/BBDPPatient/PatientInstructionServlet";
				...
				success : function(response) {
					...
					for(var number=0; number<response.commentIDList.length; number++){
						...
						if(response.hideImgList[number] == "0" || response.patientIDList[number] == patientID){	//如果是本人則顯示圖片
							...
							insertHtml += 
							"			<img src='http://140.121.196.23:3390/BBDPPatient/ProfilePictureServlet?option=getProfilePicture&patientID="+response.patientIDList[number]+"' onerror=failToLoadProfilePicture('"+response.patientIDList[number]+"'); width='100%' height='auto' id='img"+response.patientIDList[number]+"'>";
						}
